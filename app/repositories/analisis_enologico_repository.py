...
class AnalisisEnologicoRepository(BaseRepository[AnalisisEnologico]):
    model = AnalisisEnologico

    def list_by_lote(self, lote_id: int) -> list[AnalisisEnologico]:
        statement = select(self.model).where(self.model.lote_id == lote_id)
        return list(self.session.scalars(statement))

    def list_by_pileta(self, pileta_id: int) -> list[AnalisisEnologico]:
        statement = select(self.model).where(self.model.pileta_id == pileta_id)
        return list(self.session.scalars(statement))

    def list_by_bodega(self, bodega_id: int) -> list[AnalisisEnologico]:
    ...
    def latest_by_lote(self, lote_id: int) -> AnalisisEnologico | None:
        statement = (
            select(self.model)
    ...
    def latest_by_pileta(self, pileta_id: int) -> AnalisisEnologico | None:
        statement = (
            select(self.model)
